...
export const Tab: React.VFC<TabProps> = ({ label, color, index, onClick, isSelected }) => {

  const link = label === 'home' ? '' : label

  return (
    <Link to={`/${link}`} className={`link-${label}`}>
      <StyledTab color={color} index={index} onClick={onClick} isSelected={isSelected} className={`tab-${label}`}>
        {label}
      </StyledTab>
    </Link>
  )
